...
export const ConnectionsList = ({
  ...
  const subTitle = totalConnections === 0 ? 'Nenhuma conexão encontrada' : `${totalConnections} conexão${totalConnections === 1 ? '' : 'ões'} cadastrada${totalConnections === 1 ? '' : 's'}.`;
  ...
  return (
        ...
        {connections.length === 0 && (
          <div className="rounded-lg border border-dashed border-slate-200 p-6 text-center">
            ...
            <Typography className="mt-1 text-sm text-slate-500">
              Cadastre a primeira conexão para começar a organizar contatos e mensagens.
            </Typography>
          </div>
        )}
    ...
    >
      <DialogTitle id="delete-connection-title">Excluir conexão</DialogTitle>
      <DialogContent>
        <DialogContentText id="delete-connection-description">
          Tem certeza que deseja excluir a conexão {connectionToDelete?.name}? Esta ação não pode ser desfeita.
        </DialogContentText>
      </DialogContent>
      <DialogActions>
